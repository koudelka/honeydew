defmodule Honeydew.DisorderIntegrationTest do
  ...
  #   {:ok, _} = start_simple_queue(other_queue)
  ...
  defp setup_queue_name(_), do: {:ok, [queue: {:global, generate_queue_name()}]}

  defp setup_disorder_mock(%{queue: {:global, queue}}) do
    {:ok, _pid} = Honeydew.DisorderSandbox.start(queue)
    :ok
  end
  ...
  defp start_queue(queue, opts \\ []) do
    queue_opts = Keyword.merge([queue: Honeydew.Queue.Disorder], opts)
  ...
  defp start_simple_queue(queue, opts \\ []) do
